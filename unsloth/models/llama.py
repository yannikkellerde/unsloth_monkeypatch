...
BlockDiagonalCausalMask = (
    xformers.attn_bias.BlockDiagonalCausalMask if xformers is not None else None
)
...
def LlamaModel_fast_forward(
    ...
    **kwargs,
    ...
    if torch_dtype is not None:
        ...
        raise TypeError(
            "Unsloth: torch_dtype for models is not bfloat16, float16 or float32!"
        )
    pass
    ...
    if (
        ...
        position_embeddings = self.rotary_emb(
            hidden_states, position_ids, self.config.max_position_embeddings
        )
    else:
...
class FastLlamaModel:
    ...
    def from_pretrained(
        ...
        **kwargs,
        ...
        if ((post_check - pre_check) >= 1).sum() > 1 and os.environ.get(
        ...
        check_batches = "\n".join(
            [check_batches[0]] + [front_spaces + x[8:] for x in check_batches[1:]]
        )
        inner_training_loop = inner_training_loop.replace(
            ...
            if check_all:
                ...
                if "embed_tokens" in new_target_modules:
                    ...
                    model.get_input_embeddings().modules_to_save.default.to(
                        device="cuda:0", dtype=new_dtype, non_blocking=True
                    )
        ...
        if lora_dropout == 0 and bias == "none":
            for idx, layer in enumerate(model.model.model.layers):
                ...
                if (
                    ...
                    and (
                        len(getattr(gate_proj, "lora_magnitude_vector", []) or []) == 0
                    )
