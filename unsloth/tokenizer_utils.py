...
def patch_trl_tokenizer_processing_class(trainer_name):
    ...
    if str(eval(f"{trainer_name}").__name__).startswith("Unsloth"):
        return None
    ...
    args = {
        key: value.default if type(value.default) is not str else f"'{value.default}'"
...
for trainer_name in ("SFTTrainer", "DPOTrainer", "KTOTrainer"):
    ...
    if trainer_text is None:
        continue
    try:
        exec(trainer_text, globals())
    except:
        raise RuntimeError(
            f"Unsloth: Please file a bug report! Error patching {trainer_name}"
        )
